...
class App {
  ...
  _setup() {
    ...
    // Get the div container with id "renderthis"
    const container = document.getElementById("renderthis");
    container.appendChild(renderer.domElement);
    this._scene = new THREE.Scene();
    const camera = this._camera = new MousePerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.1, 1000);
  ...
  _createScene() {
    ...
    scene.add(light);
    const skybox = new Skybox([1024, 1024, 1024], [SKYBOX_IMG, SKYBOX_IMG, SKYBOX_IMG, SKYBOX_IMG, SKYBOX_IMG, SKYBOX_IMG]);
    scene.add(skybox);
...
class SpinnableObject extends THREE.Object3D {
  ...
  _projectOnTrackball(touchX, touchY) {
    ...
    mouseOnBall.set(this._clamp(touchX / (window.innerWidth / 2), -1, 1), this._clamp(-touchY / (window.innerHeight / 2), -1, 1), 0.0);
